...
{
    //받아온 캐릭터 에셋의 애니메이션 코드
    [SerializeField]
    public GameObject queryBodyParts; //캐릭터
    public Vector3 originPos; //최초 위치
    ...
    void Start()
    {
        ...
        queryBodyParts.transform.localPosition = originPos;
        queryBodyParts.transform.localRotation = originRot;
        motionAnimator.SetInteger("AnimIndex", 10); //idle 애니메이션

    }

    void Update()
    {
        if(!gameManager.isPause) //게임 내 일시정지 상태가 아닐 때에만 캐릭터 움직임
